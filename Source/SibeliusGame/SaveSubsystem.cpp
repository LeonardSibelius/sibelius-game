// SaveSubsystem.cpp — SIB-29 Ch5 Phase 1. The one place SaveGame disk I/O happens.

#include "SaveSubsystem.h"
#include "Kismet/GameplayStatics.h"
#include "GameFramework/SaveGame.h"
#include "SibeliusGame.h"   // LogSibeliusGame

bool USaveSubsystem::CommitSave(USaveGame* SaveObject, const FString& SlotName, int32 UserIndex) const
{
	if (!SaveObject || SlotName.IsEmpty())
	{
		UE_LOG(LogSibeliusGame, Warning, TEXT("[Save] CommitSave rejected: %s."),
			!SaveObject ? TEXT("null save object") : TEXT("empty slot name"));
		return false;
	}
	const bool bOk = UGameplayStatics::SaveGameToSlot(SaveObject, SlotName, UserIndex);
	UE_LOG(LogSibeliusGame, Display, TEXT("[Save] CommitSave to slot '%s': %s."),
		*SlotName, bOk ? TEXT("OK") : TEXT("FAILED"));
	return bOk;
}

USaveGame* USaveSubsystem::LoadSave(const FString& SlotName, int32 UserIndex) const
{
	if (SlotName.IsEmpty() || !UGameplayStatics::DoesSaveGameExist(SlotName, UserIndex))
	{
		return nullptr;
	}
	return UGameplayStatics::LoadGameFromSlot(SlotName, UserIndex);
}

bool USaveSubsystem::HasSave(const FString& SlotName, int32 UserIndex) const
{
	return !SlotName.IsEmpty() && UGameplayStatics::DoesSaveGameExist(SlotName, UserIndex);
}

bool USaveSubsystem::DeleteSave(const FString& SlotName, int32 UserIndex) const
{
	if (SlotName.IsEmpty() || !UGameplayStatics::DoesSaveGameExist(SlotName, UserIndex))
	{
		return false;
	}
	return UGameplayStatics::DeleteGameInSlot(SlotName, UserIndex);
}
