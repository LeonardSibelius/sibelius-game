...
public:
	...
	// --- Headless gate hooks (SS1) -----------------------------------------
	// The pool + its knobs are private/protected (PIE is the real visual gate,
	// SS8), so SauceSmokeTest reaches the pool's STATE through these. Set the
	// knobs BEFORE DispatchBeginPlay so BeginPlay builds the pool from them.
	void ConfigureForTest(int32 InPoolSize, const TArray<FVector>& InSources,
		const FVector& InMouth, const TArray<TObjectPtr<UStaticMesh>>& InMeshes)
	{
		PoolSize        = InPoolSize;
		SourceLocations = InSources;
		MouthLocation   = InMouth;
		BookMeshes      = InMeshes;
	}
	int32 GetConfiguredPoolSizeForTest() const { return PoolSize; }
	int32 GetPoolNumForTest() const { return Pool.Num(); }
	int32 GetActiveBookCountForTest() const
	{
		int32 N = 0;
		for (const FFallingBook& B : Pool) { if (B.bActive) { ++N; } }
		return N;
	}
