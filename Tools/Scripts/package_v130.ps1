# Package the v1.3.0 SHIPPING Win64 build — "The Launch".
#
# Content of 1.3.0: boarding, and what happens after it.
#
# C is the pre-flight compile (docs/SPACEPORT_PLAN.md's own row for that key). Near an
# assembled spaceport with supplies bought it takes him aboard; without them it is a
# compile error naming uFoods. C again while aboard is IGNITION: a Niagara Fluids plume
# under PackDev's SM_Rocket, four exterior cameras and the cabin, a gravity-turn climb,
# skippable — and afterward the pad is empty (branch state 3) and he is back on the lawn.
#
# Also carried: generated objects survive a level change (the spaceport stopped vanishing
# when he walked to uFoods), which 1.2.0 shipped without.
#
# THE ONE THING THIS RELEASE CAN LOSE QUIETLY. The plume is a NiagaraFluids PLUGIN
# template, hard-referenced from ASpaceport's constructor so the cooker follows it. That
# is the right shape — but plugin template content is exactly the family of asset that
# has gone missing from a pak before while working perfectly in PIE (v0.7.4, the
# invisible spaceport). The probe at the bottom looks for it in the cook. Read it.
...
$log     = "$root\pkg-v130.log"

# The pak we are growing from, so the cost of the supermarket is a number and not a feeling.
$baselinePaks = "C:\Users\wpark\builds\sibelius-v1.2.0\Windows\SibeliusGame\Content\Paks"
...
Write-Output ""
Write-Output "---------------- SIZE ----------------"
Write-Output ("pak  v1.2.0   : {0} GB" -f $old)
Write-Output ("pak  v1.3.0   : {0} GB" -f $new)
if ($old -and $new) { Write-Output ("the launch cost: {0} GB" -f [math]::Round($new - $old, 2)) }
